"""
Engine 1 -- Market Valuation Engine (MVE).
=================================================================
...
from .causal import (EPS, BatchDLM, DynamicModelAverage, EWMA, ExpandingRank,
                     OnlineAR1, norm_cdf_scalar)
from .factors import K_MAX, AdaptiveVolPanel, OnlineFactorModel
from .regime import RegimeFilter
from .universe import TICKER_CLASS
...
def _blocks(tickers: list[str]) -> tuple[list[str], np.ndarray]:
    ...
    names = sorted({TICKER_CLASS.get(t, "Other") for t in tickers})
    M = np.zeros((len(names), len(tickers)))
    idx = {b: i for i, b in enumerate(names)}
    for j, t in enumerate(tickers):
        M[idx[TICKER_CLASS.get(t, "Other")], j] = 1.0
    return names, M
...
class MarketValuationEngine:
    ...
    def run(self, target_px: pd.Series, expl_px: pd.DataFrame,
        ...
        for t in range(T):
            ...
            avail = (np.isfinite(r) & PR[t] & (n_prints[t] >= MIN_PRINTS)
                     & (sig > 1e-7))
            ...
            if lat_bank is None:
                ...
                lat_bank = DynamicModelAverage(
                    K_MAX + 1, grid=VALUATION_DELTAS, prior_scale=v0 * 25.0,
                    prior_var=v0 * 25.0, prior_mean=pm_l)
                blk_bank = DynamicModelAverage(
                    self.n_blocks + 1, grid=VALUATION_DELTAS, prior_scale=v0 * 25.0,
